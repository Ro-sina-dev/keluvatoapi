...
<head>
  ...
  <meta name="twitter:description" content="Découvrez nos meubles, décorations et solutions de bricolage pour embellir votre maison." />
  ...
  <style>
    :root { --primary:#1f4e5f; --primary-600:#0e3948; --muted:#6b7280; --border:#e5e7eb; --bg:#f8fafc; --card:#ffffff; --success:#10b981; --danger:#ef4444; --ring:rgba(31,78,95,.18); }
    *{box-sizing:border-box}
    body{margin:0;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,'Helvetica Neue',Arial;background:var(--bg);color:#111827}
    a{color:inherit}
    .container{max-width:1100px;margin:28px auto;padding:0 18px}
    .page-title{margin:0 0 14px;font-size:28px;letter-spacing:.2px}
    .muted{color:var(--muted)}
    .stepper{position:relative;display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin:10px 0 22px}
    .stepper::before{content:"";position:absolute;left:10px;right:10px;top:19px;height:2px;background:#eaecef;z-index:0}
    .step{position:relative;display:flex;flex-direction:column;align-items:center;gap:8px;z-index:1}
    .step-dot{width:28px;height:28px;border-radius:999px;border:2px solid var(--primary);display:grid;place-items:center;font:700 13px/1 system-ui;color:var(--primary);background:#fff}
    .step.active .step-dot,.step.completed .step-dot{background:var(--primary);color:#fff}
    .step-label{font-weight:600;font-size:13px;color:#374151}
    .step:not(.active):not(.completed){opacity:.6}
    .layout{display:grid;grid-template-columns:1fr 340px;gap:22px}
    @media (max-width:960px){.layout{grid-template-columns:1fr}}
    .card{background:var(--card);border:1px solid var(--border);border-radius:14px;box-shadow:0 6px 20px rgba(17,24,39,.04)}
    .card-pad{padding:18px}
    .card+.card{margin-top:14px}
    .cart-item{display:grid;grid-template-columns:66px 1fr auto;gap:12px;align-items:center;padding:10px 0;border-bottom:1px solid #f2f2f2}
    .cart-item:last-child{border-bottom:none}
    .cart-item img{width:66px;height:66px;object-fit:cover;border-radius:10px;border:1px solid #eee}
    ...
    .btn-ghost{width:30px;height:30px;border:1px solid var(--border);background:#fff;border-radius:9px;cursor:pointer}
    .btn-remove{margin-left:8px;border:1px solid var(--border);background:#fff;border-radius:9px;padding:7px 10px;cursor:pointer}
    .form-group{margin:12px 0}
    .form-row{display:grid;grid-template-columns:1fr 1fr;gap:12px}
    @media (max-width:640px){.form-row{grid-template-columns:1fr}}
    .label{display:block;margin:0 0 6px;font-size:14px;color:#374151;font-weight:600}
    .control,select,textarea{width:100%;padding:11px 12px;border:1px solid var(--border);border-radius:10px;background:#fff;outline:none}
    .control:focus,select:focus,textarea:focus{border-color:var(--primary);box-shadow:0 0 0 4px var(--ring)}
    .pay-method{border:1px solid var(--border);border-radius:12px;padding:12px;transition:.15s}
    .pay-method.active{outline:2px solid var(--primary);outline-offset:0;background:#f9fbfc}
    .pay-head{display:flex;align-items:center;gap:10px}
    .pay-head img{height:22px;margin-left:auto;opacity:.85}
    .pay-details{margin-top:10px}
    .btn{cursor:pointer;border:1px solid var(--border);background:#fff;border-radius:12px;padding:11px 14px;font-weight:600}
    ...
    aside.summary{position:sticky;top:20px;align-self:start}
    ...
    .confirm{text-align:center;padding:12px 10px}
    .confirm .ico{font-size:66px;color:var(--success);line-height:1;margin:6px 0}
    .order-kv{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #eef0f3}
    .order-kv:last-child{border-bottom:none}

    /* >>> Stripe Checkout gère les cartes, on masque les faux champs */
...
<div class="container">
  ...
  <p class="muted" style="margin:0 0 12px">Vérifiez votre panier, renseignez la livraison, puis payez en toute sécurité.</p>

  <!-- Stepper -->
  <div class="stepper">
    <div class="step active" id="stp1"><div class="step-dot">1</div><div class="step-label">Panier</div></div>
    <div class="step" id="stp2"><div class="step-dot">2</div><div class="step-label">Livraison</div></div>
    <div class="step" id="stp3"><div class="step-dot">3</div><div class="step-label">Paiement</div></div>
    <div class="step" id="stp4"><div class="step-dot">4</div><div class="step-label">Confirmation</div></div>
  </div>
  ...
  <div class="layout">
    ...
    <main>
      ...
      <section class="card card-pad" id="step-cart">
        ...
        <p id="cart-empty" class="muted" style="display:none;margin:0 0 8px">
          Votre panier est vide. <a href="{{ route('home') }}" class="link" style="color:#2563eb;text-decoration:none">Retour à la boutique</a>
        </p>
      ...
      <section class="card card-pad" id="step-payment" style="display:none">
        ...
        <div class="pay-method active" id="pm-card">
          <div class="pay-head">
            ...
            <img src="https://upload.wikimedia.org/wikipedia/commons/0/04/Credit_card_fonts_and_logos.png" alt="Cartes">
          </div>
          <!-- Masqué: Stripe Checkout gère la saisie -->
          <div class="pay-details" id="card-fields">
            <!-- champ masqué volontairement -->
          </div>
        </div>
        ...
        <div class="pay-method" id="pm-cod">
          <div class="pay-head">
            ...
            <img src="https://upload.wikimedia.org/wikipedia/commons/3/3c/Money_cash.jpg" alt="Cash">
          </div>
        </div>

        <div class="form-group" style="margin-top:16px">
          <input type="checkbox" id="terms-agree">
          <label for="terms-agree">J’accepte les <a href="#" style="color:#2563eb;text-decoration:none">conditions générales de vente</a></label>
        </div>
      ...
      <!-- Étape 4: Confirmation (conservée pour d'autres flux si besoin) -->
      <section class="card card-pad" id="step-confirm" style="display:none">
        <div class="confirm">
          ...
          <p id="cf-text-2" class="muted" style="margin:2px 0 14px">Votre commande a été enregistrée.</p>
        </div>
    ...
    <!-- Formulaire caché pour Stripe Checkout -->
    <form id="stripeForm" action="{{ route('checkout.stripe') }}" method="POST" style="display:none;">@csrf</form>
      ...
      <div class="card card-pad">
        <div class="form-group" style="margin:0">
          ...
          <div style="display:flex;gap:8px">
            ...
            <button id="btn-coupon" class="btn btn-primary" style="white-space:nowrap">Appliquer</button>
          </div>
          <div class="muted" style="margin-top:6px">Astuce : utilisez <strong>KELU15</strong> (−15%)</div>
        </div>
      </div>
    </aside>
  </div>
...
(() => {
  // ========= Config (sans API) =========
  const CURRENCY = 'EUR';
  const fmt = (n, c=CURRENCY) => new Intl.NumberFormat('fr-FR', {style:'currency', currency:c}).format(+n||0);

  function normalizeImg(u){
    if(!u) return '';
    if(typeof u!=='string'){ try{ u=String(u) }catch{ return '' } }
    u = u.replace(/^https?:\/\/(127\.0\.0\.1(:\d+)?|localhost(:\d+)?)/i,'');
    if(/^(https?:\/\/|data:)/i.test(u)) return u;
    if(u.startsWith('/storage/')) return window.location.origin+u;
    return window.location.origin+'/storage/'+u.replace(/^\/+/,'');
  }
  ...
  const Cart = {
    ...
    subtotal(items=this.get()){ return items.reduce((t,i)=>t+(+i.price||0)*(+i.qty||0),0) },
    updateQty(id,q){
      const arr=this.get(); const it=arr.find(x=>String(x.id)===String(id)); if(!it) return;
      it.qty=Math.max(0,+q||0); if(it.qty===0) arr.splice(arr.indexOf(it),1);
      this.set(arr); renderCart(); updateSummary();
    },
    remove(id){ const arr=this.get().filter(x=>String(x.id)!==String(id)); this.set(arr); renderCart(); updateSummary() },
    clear(){ localStorage.setItem(this.KEY,'[]') }
  };

  function normalizeCartData(){
    const arr=Cart.get(); let changed=false;
    arr.forEach(it=>{
      if(it.quantity!=null && it.qty==null){ it.qty=+it.quantity||1; delete it.quantity; changed=true; }
      if(!it.image && it.img){ it.image=it.img; changed=true; }
      if(!it.currency){ it.currency='EUR'; changed=true; }
    });
    if(changed) Cart.set(arr);
  }

  // ========= Étapes & Stepper =========
  const steps=['step-cart','step-delivery','step-payment','step-confirm'].map(id=>document.getElementById(id));
  ...
  function totals(){
    const sub=Cart.subtotal(); const disc=coupon? sub*(coupon.percent/100):0;
  ...
  function renderCart(){
    ...
    if(!items.length){ cartEmpty.style.display='block'; cartList.innerHTML=''; return; }
    cartEmpty.style.display='none';
    cartList.innerHTML = items.map(it=>{
      const q=+it.qty||0, line=(+it.price||0)*q, imgSrc=normalizeImg(it.image)||'https://via.placeholder.com/66?text=IMG';
      return `
        <div class="cart-item" data-id="${it.id}">
          <img src="${imgSrc}" alt="${(it.name||'').replace(/"/g,'&quot;')}">
          <div>
    ...
    cartList.querySelectorAll('.cart-item').forEach(row=>{
      ...
      row.querySelector('[data-act="inc"]').addEventListener('click', ()=>{
        const it=Cart.get().find(x=>String(x.id)===String(id)); Cart.updateQty(id,(+it?.qty||0)+1);
      });
  ...
  document.getElementById('btn-coupon').addEventListener('click', ()=>{
    ...
    if(!code){ coupon=null; updateSummary(); alert('Aucun code saisi.'); return; }
    if(code==='KELU15'){ coupon={code, percent:15}; updateSummary(); alert('Réduction de 15% appliquée ✅'); }
    else{ coupon=null; updateSummary(); alert('Code invalide ❌'); }
  });
  ...
  // Toggle méthodes de paiement (on ne montre jamais #card-fields)
  ...
  [pmrCard, pmrPayPal, pmrCod].forEach(r=>{
    r.addEventListener('change', ()=>{
      ...
      // Stripe Checkout gère la saisie:
  ...
  document.getElementById('go-payment').addEventListener('click', ()=>{
    ...
    if(!name || !addr || !city || !zip || !phone){ alert('Merci de compléter tous les champs requis de livraison.'); return; }
    goStep(3);
  });
  ...
  async function syncCartWithServer(){
    ...
    }catch(e){ console.error('Erreur sync panier:', e); }
  }

  // ========= Init =========
  function init(){ normalizeCartData(); renderCart(); updateSummary(); syncCartWithServer(); goStep(1); }
  init();
})();
</script>

<!-- Handler UNIFIÉ: persiste livraison puis lance Stripe / gère COD/PayPal -->
<script>
async function persistDelivery(){
  ...
  // Validation rapide
  for(const k of ['name','address','city','zip','country','phone']){
    if(!body.get(k)) throw new Error('Merci de compléter tous les champs de livraison.');
  }

  const res = await fetch('{{ route('checkout.delivery.save') }}', {
    method:'POST',
    headers:{
  ...
  if(!res.ok){
    ...
    throw new Error('Impossible d’enregistrer la livraison : '+txt);
  }
}

document.getElementById('do-pay').addEventListener('click', async ()=>{
  ...
  try{
    // Toujours persister la livraison avant de payer
    await persistDelivery();

    if(pm === 'card'){
      document.getElementById('stripeForm').submit(); // redirection vers Stripe Checkout
      return;
    }
    if(pm === 'cod'){
      alert('Paiement à la livraison : la commande sera réglée à réception.');
      // Ici, tu peux rediriger vers un endpoint "place" si tu veux créer une commande COD côté serveur.
      return;
    }
    if(pm === 'paypal'){
      alert('PayPal non encore activé.');
      return;
    }
  }catch(e){
    alert(e.message || 'Erreur pendant la préparation du paiement.');
  }
});
</script>
