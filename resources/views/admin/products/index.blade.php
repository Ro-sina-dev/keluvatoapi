...
<div class="app">
    ...
    <div class="cardx">
        <div class="hd">
            ...
            <a href="{{ route('admin.products.create') }}" class="btn-brand">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            @if(!empty($product->images) && count($product->images) > 0)
                                <img src="{{ asset('storage/' . $product->images[0]) }}" alt="{{ $product->name }}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 8px;">
                            @else
                                <div style="width: 50px; height: 50px; background: var(--card); border-radius: 8px; display: flex; align-items: center; justify-content: center;">
                        ...
                        <td>
                            <span class="badge-soft {{ $product->stock > 0 ? 'text-success' : 'text-danger' }}">
                        ...
                        <td>
                            <span class="badge-soft {{ $product->is_active ? 'text-success' : 'text-danger' }}">
                                {{ $product->is_active ? 'Actif' : 'Inactif' }}
                            </span>
                        </td>
                        <td>
                            <div class="d-flex gap-2">
                                <a href="{{ route('admin.products.edit', $product) }}" class="btn-outline btn-sm">
                                ...
                                <form action="{{ route('admin.products.destroy', $product) }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer ce produit ?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn-outline btn-sm text-danger">
                    ...
                    <tr>
                        <td colspan="8" class="text-center py-4">
                            ...
                            <a href="{{ route('admin.products.create') }}" class="btn-brand mt-3">
                                <i class="fa-solid fa-plus"></i> Ajouter votre premier produit
                            </a>
                        </td>
                    </tr>
                    @endforelse
