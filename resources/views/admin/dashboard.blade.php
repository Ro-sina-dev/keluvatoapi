...
<body>
  ...
  <script>
    ...
    // ==== Admin guard (déconnecte si non-admin)
    ...
    async function loadStats() {
      try {
        ...
        // KPIs
        document.getElementById("kpiRevenue").textContent = money(
          data.kpis?.revenue || 0
        );
        document.getElementById("kpiOrders").textContent = nf(
          data.kpis?.orders || 0
        );
        document.getElementById("kpiUsers").textContent = nf(
          data.kpis?.users || 0
        );
        document.getElementById("kpiProducts").textContent = nf(
          data.kpis?.products || 0
        );
        document.getElementById("countAdmins").textContent = nf(
          data.kpis?.admins || 0
        );
        document.getElementById("countPros").textContent = nf(
          data.kpis?.pros || 0
        );
        document.getElementById("countUsers").textContent = nf(
          (data.kpis?.users || 0) -
          (data.kpis?.pros || 0) -
          (data.kpis?.admins || 0)
        );
        ...
        // Charts
        const months = (data.sales_by_month || []).map(
          (x) => x.ym?.slice(5, 7) + "/" + x.ym?.slice(2, 4)
        );
        const sales = (data.sales_by_month || []).map((x) => x.revenue || 0);
        buildSalesChart("line", months, sales);

        const topLabels = (data.top_products || []).map((x) => x.name);
        const topValues = (data.top_products || []).map((x) => x.qty);
        buildTopProductsChart(topLabels, topValues);
        ...
        fillLowStock(data.low_stock || []);
        fillTopList(data.top_products || []);
        fillOrdersTable(data.recent_orders || []);
      } catch (e) {
    ...
    // Boot: vérifie admin puis charge
    (async () => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 1) Charger les catégories (racines + enfants)
  (async function loadCategories(){
    try {
      const res = await fetch(`${API_BASE}/categories`, {
        headers: { 'Accept': 'application/json' }
      });
      const roots = await res.json(); // [{id,name,children:[...]}]
  ...
  form.addEventListener('submit', async (e) => {
    e.preventDefault();
    const fd = new FormData(form); // inclut name, price, stock, description, category_id, files[]
    ...
    try {
      const res = await fetch(`${API_BASE}/products`, {
        ...
        body: fd // ne PAS fixer Content-Type avec FormData
      });

      const data = await res.json().catch(() => ({}));

      if (!res.ok) {
        // Affiche proprement les erreurs de validation si dispo
