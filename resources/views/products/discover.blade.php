...
<body>
    ...
    <section class="discover-hero">
        <div class="container hero-wrap">
            <div class="hero-head">
                ...
                <p>Trouvez l'inspiration parmi nos meubles, décorations et outils de bricolage pour créer l'intérieur de vos rêves.</p>
                <div class="hero-cta">
        ...
        <div class="promo-stage">
            ...
            <div class="promo-slider" aria-label="promotions">
                <article class="promo a">
                    ...
                    <img loading="lazy" src="https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=1200&q=80&auto=format&fit=crop" alt="Salon promo">
                </article>
                <article class="promo b">
                    ...
                    <img loading="lazy" src="https://images.unsplash.com/photo-1540932239986-30128078f3c5?w=1200&q=80&auto=format&fit=crop" alt="Luminaires">
                </article>
                <article class="promo c">
                    ...
                    <img loading="lazy" src="https://images.unsplash.com/photo-1600585154526-990dced4db0d?w=1200&q=80&auto=format&fit=crop" alt="Jardin">
                </article>
            </div>
            ...
            <form method="get" action="{{ route('products.discover') }}" class="sidebar-card k-glow">
                ...
                <div class="filter-group">
                    ...
                    <div class="filter-body row-2">
                        <div class="field">
                            <label>Min</label>
                            <input type="number" step="0.01" name="price_min" value="{{ request('price_min') }}" placeholder="0.00">
                        </div>
                        <div class="field">
                            <label>Max</label>
                            <input type="number" step="0.01" name="price_max" value="{{ request('price_max') }}" placeholder="9999.00">
                        </div>
                    </div>
                </div>
                ...
                <div class="filter-group">
                    ...
                    <div class="filter-body stars">
                        @foreach([4,3,2,1] as $r)
                            <label>
                                <input type="radio" name="rating" value="{{ $r }}" @checked((int)request('rating')===$r)>
                                @for($i=1;$i<=5;$i++)
                    ...
                    <div class="filter-group">
                        ...
                        <div class="filter-body list">
                            @foreach($brands as $b)
                                ...
                                <label class="row">
                                    <input type="checkbox" name="brand[]" value="{{ $b->slug }}" @checked($checked)>
                                    <span>{{ $b->name }}</span>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div class="filter-body colors">
                        @foreach($colors as $c)
                            ...
                            <label class="swatch {{ $isSel ? 'active' : '' }}" style="--sw: {{ $c }}">
                                <input type="checkbox" name="color[]" value="{{ $c }}" @checked($isSel)>
                            </label>
                        @endforeach
                    </div>
                </div>
                ...
                @forelse($products as $p)
                    ...
                    <article class="card" data-id="{{ $p->id }}">
                        <div class="media">
                            ...
                            @if ($hasPromo)
                                <span class="badge">-{{ number_format((1 - $p->discount_price / $p->price) * 100, 0) }}%</span>
                        ...
                        <div class="info">
                            ...
                            <div class="price">
                                <span data-price>{{ number_format($display, 2, ',', ' ') }} {{ $p->currency ?? 'EUR' }}</span>
                                @if ($hasPromo)
                                    <span class="old">{{ number_format($p->price, 2, ',', ' ') }} {{ $p->currency ?? 'EUR' }}</span>
                                @endif
                            </div>
                            <div class="meta">
        ...
        <div class="container">
            ...
            <div class="slide-row">
                @foreach ($featured as $f)
                    @php
                        $imgs = (array) ($f->images ?? []);
                        $img = count($imgs) ? $imgs[0] : 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=800&q=80&auto=format&fit=crop';
                    @endphp
                    <article class="slide">
                        <img loading="lazy" src="{{ $img }}" alt="{{ $f->name }}" style="width:100%;aspect-ratio:16/9;object-fit:cover">
                        <div style="padding:12px 14px">
                            <strong>{{ $f->name }}</strong>
                            <div style="color:var(--muted);font-size:14px">{{ number_format($f->price, 2, ',', ' ') }} {{ $f->currency ?? 'EUR' }}</div>
                        </div>
                    </article>
                @endforeach
            </div>
        ...
        <div class="container">
            ...
            <div class="slide-row">
                @foreach ($promos as $s)
                    @php
                        $imgs = (array) ($s->images ?? []);
                        $img = count($imgs) ? $imgs[0] : 'https://images.unsplash.com/photo-1540574163026-643ea20ade25?w=800&q=80&auto=format&fit=crop';
                    @endphp
                    <article class="slide">
                        <img loading="lazy" src="{{ $img }}" alt="{{ $s->name }}" style="width:100%;aspect-ratio:16/9;object-fit:cover">
                        <div style="padding:12px 14px">
                            <strong>{{ $s->name }}</strong>
                            <div>
                                <span style="font-weight:700">{{ number_format($s->discount_price ?? $s->price, 2, ',', ' ') }} {{ $s->currency ?? 'EUR' }}</span>
                                @if ($s->discount_price)
                                    <span class="old">{{ number_format($s->price, 2, ',', ' ') }} {{ $s->currency ?? 'EUR' }}</span>
                                @endif
                            </div>
                        </div>
                    </article>
    ...
    <script>
        ...
        (function(){
            ...
            const toggle = (el) => { if (!el) return; el.style.display = el.style.display === "block" ? "none" : "block"; }
            const hideAll = () => [dropdownUser, dropdownHelp].forEach(d => { if (d) d.style.display = "none"; });

            if (userToggle && dropdownUser){
                userToggle.addEventListener("click",(e)=>{ e.stopPropagation(); hideAll(); toggle(dropdownUser); });
            }
            if (helpToggle && dropdownHelp){
                helpToggle.addEventListener("click",(e)=>{ e.stopPropagation(); hideAll(); toggle(dropdownHelp); });
            }
        ...
        (function () {
            ...
            const firstImage = (images) => {
                ...
                if (typeof images === 'string') {
                    try { const arr = JSON.parse(images); if (Array.isArray(arr) && arr[0]) return arr[0]; } catch(e){}
                }
                return '';
            };
            ...
            function render(q, data){
                ...
                if (allLink) allLink.addEventListener('click', (e)=>{ e.preventDefault(); window.location.href = `recherche.html?q=${encodeURIComponent(q)}`; });
            }
            ...
            const doSearch = debounce(async (q) => {
                ...
                try {
                    const res  = await fetch(`${apiBase}/search?q=${encodeURIComponent(q)}`, { headers: {Accept:'application/json'} });
                    const data = await res.json();
                    if (q !== lastQ) return;
            ...
            btn.addEventListener('click', (e)=>{ e.preventDefault(); const q = input.value.trim(); if (!q) return; window.location.href = `recherche.html?q=${encodeURIComponent(q)}`; });

            input.addEventListener('keydown', (e)=>{
                if (dd.style.display !== 'block') return;
                const max = items.length - 1;
                if (e.key === 'ArrowDown'){ e.preventDefault(); cursor = (cursor < max) ? cursor + 1 : 0; updateActive(); }
                else if (e.key === 'ArrowUp'){ e.preventDefault(); cursor = (cursor > 0) ? cursor - 1 : max; updateActive(); }
                else if (e.key === 'Enter'){
                    e.preventDefault();
                    if (cursor >=0 && items[cursor]) window.location.href = `product.html?id=${encodeURIComponent(items[cursor].id)}`;
                    else if (input.value.trim()) window.location.href = `recherche.html?q=${encodeURIComponent(input.value.trim())}`;
                } else if (e.key === 'Escape'){ dd.style.display='none'; }
            });
            function updateActive(){
                dd.querySelectorAll('.search-item').forEach((el,i)=>{
                    if (i===cursor){ el.classList.add('active'); el.scrollIntoView({block:'nearest'}); }
        ...
        (function(){
            ...
            function updateCartDisplay(){
                ...
                cart.forEach((item)=>{
                    ...
                    row.innerHTML = `<span>${item.name} x${item.quantity}</span><span>${euro(itemTotal)}</span>`;
            ...
            if (cartLink){
                cartLink.addEventListener("click", (e)=>{
                    ...
                    if (!userToken){ window.location.href = "{{ route('login') }}"; return; }
                    if (checkoutModal){ updateCartDisplay(); checkoutModal.style.display = "block"; }
        ...
        (function(){
            ...
            window.addEventListener("scroll", function(){
                ...
                if (scrollTop > lastScrollTop){ header.style.top = "-150px"; }
        ...
        (function(){
            const translations = {
                fr:{search_placeholder:"Rechercher des meubles, décoration...", select_country:"Sélectionnez votre pays", search_country:"Rechercher un pays..."},
                en:{search_placeholder:"Search for furniture, decor...",      select_country:"Select your country",      search_country:"Search for a country..."},
                es:{search_placeholder:"Buscar muebles, decoración...",        select_country:"Selecciona tu país",       search_country:"Buscar un país..."}
            };
            window.changeLanguage = function(lang){
                const codeEls = document.querySelectorAll(".language-code");
                codeEls.forEach((el)=> el.textContent = (lang||'fr').toUpperCase());
                document.getElementById("dropdownLanguage")?.style && (document.getElementById("dropdownLanguage").style.display="none");

                document.querySelectorAll(".translate").forEach((el)=>{
                    const key = el.getAttribute("data-key");
                    el.textContent = (translations[lang] && translations[lang][key]) || (translations['fr'][key]||'');
                });
                const searchInput = document.querySelector(".search-container input");
                if (searchInput) searchInput.placeholder = translations[lang]?.search_placeholder || translations['fr'].search_placeholder;
                const countrySearch = document.getElementById("countrySearch");
                if (countrySearch) countrySearch.placeholder = translations[lang]?.search_country || translations['fr'].search_country;
            ...
            document.addEventListener("DOMContentLoaded", function(){
                ...
                languageToggle?.addEventListener("click",(e)=>{ e.stopPropagation(); if (dropdownLanguage) dropdownLanguage.style.display = dropdownLanguage.style.display==="block"?"none":"block"; });
                document.addEventListener("click",(e)=>{ if (!e.target.closest(".language-dropdown")) { if (dropdownLanguage) dropdownLanguage.style.display = "none"; } });
            });
        })();
        ...
        (function(){
            ...
            document.getElementById("mobileRegionForm")?.addEventListener("submit", function(e){
                ...
                localStorage.setItem("userSettings", JSON.stringify({currency, country, language}));
            ...
            window.updateRegionalSettings = function(currency, country, language){
                console.log("Paramètres mis à jour:", {currency, country, language});
            };
        })();
        ...
        (function(){
            ...
            document.getElementById("apply-coupon")?.addEventListener("click", function(){
                ...
                if (couponCode.toUpperCase() === "KELU15"){
                    const subtotal = parseFloat((subtotalElement.textContent||'0').replace(/[€\s]/g,"")) || 0;
                    ...
                    discountElement.innerHTML = `<span>Réduction (15%)</span><span>-€${discount.toFixed(2)}</span>`;
            ...
            document.getElementById("proceed-to-checkout")?.addEventListener("click", ()=> alert("Fonctionnalité de paiement à implémenter"));

            document.querySelector(".floating-buttons button:nth-child(1)")?.addEventListener("click", ()=> alert("Partage à implémenter"));
            document.querySelector(".floating-buttons button:nth-child(2)")?.addEventListener("click", ()=> alert("Cagnotte à implémenter"));

            document.getElementById("proceed-to-delivery")?.addEventListener("click", function(){
                document.getElementById("checkout-modal")?.style && (document.getElementById("checkout-modal").style.display = "none");
                document.getElementById("delivery-modal")?.style && (document.getElementById("delivery-modal").style.display = "block");
            });
            document.getElementById("proceed-to-payment")?.addEventListener("click", function(){
                document.getElementById("delivery-modal")?.style && (document.getElementById("delivery-modal").style.display = "none");
                document.getElementById("payment-modal")?.style && (document.getElementById("payment-modal").style.display = "block");
            });
            document.getElementById("confirm-order")?.addEventListener("click", function(){
                document.getElementById("payment-modal")?.style && (document.getElementById("payment-modal").style.display = "none");
                document.getElementById("confirmation-modal")?.style && (document.getElementById("confirmation-modal").style.display = "block");
            });
            document.getElementById("back-to-cart")?.addEventListener("click", function(){
                document.getElementById("delivery-modal")?.style && (document.getElementById("delivery-modal").style.display = "none");
                document.getElementById("checkout-modal")?.style && (document.getElementById("checkout-modal").style.display = "block");
            });
            ...
            document.getElementById("return-to-shop")?.addEventListener("click", function(){
                document.getElementById("confirmation-modal")?.style && (document.getElementById("confirmation-modal").style.display = "none");
        ...
        (function(){
            ...
            document.addEventListener('click', (e)=>{
                ...
                if (!within){ sidebar.classList.remove('open'); document.body.style.overflow=''; }
            });
        ...
        (function(){
            class ARMeasurementApp {
                constructor(){
                    this.isActive=false; this.measurementPoints=[]; this.measurements=[]; this.pixelsPerMM=2; this.videoStream=null;
                    this.elements = {
                ...
                async startAR(){
                    try{
                        ...
                        const constraints = { video:{ facingMode:"environment", width:{ideal:1920,min:640}, height:{ideal:1080,min:480} } };
                        let stream;
                        try { stream = await navigator.mediaDevices.getUserMedia(constraints); }
                        catch (error) {
                            if (error.name==="OverconstrainedError"||error.name==="NotFoundError"){
                                stream = await navigator.mediaDevices.getUserMedia({ video:{ facingMode:"user", width:{ideal:1280,min:640}, height:{ideal:720,min:480} } });
                            } else { throw error; }
                        }
                        this.videoStream = stream;
                        if (this.elements.video){ this.elements.video.srcObject = stream; }
                ...
                stopAR(){
                    ...
                    this.isActive=false; this.clearMeasurements();
                }
                calibrateScale(){
                ...
                addPointAtPosition(x,y){
                    ...
                    const point = { id: Date.now(), x, y, timestamp: new Date() };
                ...
                calculateAndRenderMeasurement(){
                    ...
                    line.style.width = `${length}px`; line.style.left = `${p1.x}px`; line.style.top = `${p1.y}px`;
                    line.style.transform = `rotate(${angle}deg)`; line.style.transformOrigin = "0 50%";
                    const label = document.createElement("div");
                    label.className = "measurement-label";
                    const displayDistance = realDistance >= 100 ? `${(realDistance/100).toFixed(2)} m` : `${realDistance.toFixed(1)} cm`;
                    label.textContent = displayDistance;
                    label.style.left = `${(p1.x + p2.x)/2}px`; label.style.top = `${(p1.y + p2.y)/2 - 30}px`;
                ...
                clearMeasurements(){
                    this.measurementPoints = []; this.measurements = [];
                    if (!this.elements.overlay) return;
                    this.elements.overlay.querySelectorAll(".measurement-point, .measurement-line, .measurement-label").forEach(el=>el.remove());
                    this.updateUI();
                }
                updateUI(){
                    if (this.elements.pointCount) this.elements.pointCount.textContent = this.measurementPoints.length;
                    const instructionEl = document.getElementById("instructionText");
                    if (!instructionEl) return;
                    if (this.measurementPoints.length === 0){ instructionEl.innerHTML = "📍 Cliquez sur le <strong>premier coin</strong> à mesurer"; instructionEl.style.color="#4a6bff"; }
                    else if (this.measurementPoints.length === 1){ instructionEl.innerHTML = "📍 Maintenant cliquez sur le <strong>second coin</strong>"; instructionEl.style.color="#ff4757"; }
                    else { instructionEl.innerHTML = "✅ Mesure terminée ! Vous pouvez en ajouter d'autres"; instructionEl.style.color="#2ed573"; }
                }
                showStatus(msg){ if (this.elements.statusText) this.elements.statusText.textContent = msg; this.elements.statusIndicator?.classList.remove("hidden"); }
                hideStatus(){ this.elements.statusIndicator?.classList.add("hidden"); }
            }
            // N'initialise l'app que si le bouton AR existe (page/section présente)
            document.addEventListener("DOMContentLoaded", ()=>{ if (document.getElementById("arMeasureBtn")) new ARMeasurementApp(); });
        })();
    </script>
